...
function safeTheme(input) {
  const theme = { ...DEFAULT_THEME, ...(isPlainObject(input) ? input : {}) };
  for (const token of ['paper', 'ink', 'muted', 'accent']) {
    if (!/^#[0-9a-f]{6}$/i.test(theme[token])) fail(`theme.${token} must be a six-digit hex colour.`);
  }
  if (typeof theme.font !== 'string' || !/^[a-z0-9 ,_'"-]{2,120}$/i.test(theme.font)) {
    fail('theme.font contains unsupported characters. Use a local/system font stack.');
  }
  return theme;
}
...
export function validateSpec(raw) {
  ...
  const theme = safeTheme(raw.theme);
  ...
  return {
    version: '1.0', family, preset, title, subtitle, source, theme, data: validatedData,
  };
}
...
function baseParts(spec) {
  ...
  const footer = 58;
  return { width, height, margin, header, footer, contentW: width - margin * 2, contentH: height - header - footer };
}
...
export function renderSvg(specInput) {
  ...
  <style>
    ...
    .eyebrow,.axis,.edge-label{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase;letter-spacing:.1em;fill:var(--muted)}
    .eyebrow{font-size:11px;font-weight:700}.axis{font-size:11px;font-weight:650}.edge-label{font-size:10px;font-weight:650}
  </style>
  <defs><marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><path d="M0 0L8 3L0 6Z" fill="${spec.theme.muted}"/></marker></defs>
  <rect width="100%" height="100%" fill="var(--paper)"/>
  <text x="${box.margin}" y="${box.margin + 10}" font-size="${titleSize}" font-weight="760" letter-spacing="-.02em">${x(spec.title)}</text>
  ...
  return rendered
    .replaceAll('var(--paper-2)', spec.theme.paper)
